...
-- The left join returns all the selects in the first table(the FROM table) and 
-- the corresponding info requested from the second table after the records are joined using a common field
-- If that info does not exist in the second table it returns a null value.
-- The inner join returns only the records that have the required selects in both tables. 
-- If one of the tables does not have a value the entire joined record will be ommited. 
